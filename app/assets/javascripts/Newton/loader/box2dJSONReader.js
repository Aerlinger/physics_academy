 ...
 *
 * @param jsonData The object (as a json hash) to be loaded
 * @param CanvasJQueryElm The jquery selector of the HTML5 canvas_width element
 */
function loadBox2dFromJSON(jsonPath, CanvasJQueryElm, onComplete) {
  ...
  $.getJSON(jsonPath, function(jsonData) {
    ...
    if(rigidBodiesElemJSON) {
      ...
      for (var i=0; i<rigidBodiesElemJSON.length; ++i) {
        ...
        model.imagePath = splicePaths(jsonPath, currentRigidBodyJSON.imagePath);
  ...
  function readAndCreateCircle(world, circleElemJSON, scale) {
    ...
    newCircle.imagePath = splicePaths(jsonPath, circleElemJSON.imagePath);
